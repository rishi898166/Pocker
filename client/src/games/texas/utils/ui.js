...
const renderNetPlayerEarnings = (
  ...
  if (win) {
    ...
    addNotification(
      username,
      "You have won a Texas Hold'Em round & earned " +
        netChipEarnings +
        " chips"
    );
  }
  if (netChipEarnings < 0) {
    addNotification(
      username,
      "You have lost a Texas Hold'Em round & lost " + netChipEarnings + " chips"
    );
  }
  return (
    <div
...
const renderActionMenu = (
  ...
  return phase === "betting1" ||
    ...
    players[activePlayerIndex].id !== uid ? (
      <h4> {`Current Move: ${players[activePlayerIndex].name}'s turn`}</h4>
    ) : (
      <React.Fragment>
        <Slider
