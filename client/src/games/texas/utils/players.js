...
import { getLobby, getUserDatabyUID } from "../../../firebase.js";
import {
  handlePhaseShift,
  reconcilePot,
  anteUpBlinds,
...
const generateTable = async (id) => {
  console.log(id);
  const table = await getLobby(id);
  let users = [];

  console.log("Table Data : ", table);
  for (let i = 0; i < table.members.length; i++) {
    const element = table.members[i];
    const userData = await getUserDatabyUID(element);
    console.log(userData);

    let temp = {
      id: userData.data.id,
      name: userData.id,
      avatarURL: userData.data.imageUrl,
      cards: [],
      showDownHand: {
        hand: [],
        descendingSortHand: [],
      },
      chips: table.isPublic ? 50000 : userData.data.fund,
      roundStartChips: table.isPublic ? 50000 : userData.data.fund,
      roundEndChips: table.isPublic ? 50000 : userData.data.fund,
  ...
  // const users = [{
  // 	id: "5cOIxxRV4FOVAfcuytYSu0yMzlm2",
  // 	name: 'thebadvulture',
  ...
  // 	robot: false
  // },{
  // 	id: "YcGwEGybyxcm75Yat4zoHvyeUG22",
  // 	name: 'test',
  // 	avatarURL: '/assets/boy.svg',
  // 	cards: [],
  // 	showDownHand: {
  ...
  // const response = await axios.get(`https://randomuser.me/api/?results=3&nat=us,gb,fr`);
...
const beginNextRound = (state) => {
  ...
  // eslint-disable-next-line
  const { players } = state; // eslint-disable-next-line
